    ...
    try {
      ...
      if (!unreadResults.isEmpty()) {
        ...
        if (result.hasTargets()) {
          ...
          latestEstimatedRobotPose.ifPresent(
              est -> {

                // if(orientation.equals(Orientation.FRONT)){
                //     inputs.latestEstimatedRobotPose =
                // latestEstimatedRobotPose.get().estimatedPose;
                // }
                ...
                inputs.latestEstimatedRobotPose =
                    latestEstimatedRobotPose
                        .get()
                        .estimatedPose
                        // Rotate by 180 to account for camera being on back, needs to be come
                        // parameter in constructor later
                        .transformBy(
                            new Transform3d(new Translation3d(), new Rotation3d(0.0, 0.0, 0.0)));
                // }

                ArrayList<Transform3d> tagTs = new ArrayList<>();
                double[] ambiguities =
                    new double[latestEstimatedRobotPose.get().targetsUsed.size()];
                if (latestEstimatedRobotPose.get().targetsUsed.size() > 0) {
                  for (int i = 0; i < latestEstimatedRobotPose.get().targetsUsed.size(); i++) {
                    tagTs.add(
                        latestEstimatedRobotPose.get().targetsUsed.get(i).getBestCameraToTarget());
                    ambiguities[i] =
                        latestEstimatedRobotPose.get().targetsUsed.get(i).getPoseAmbiguity();
                  }
                }
